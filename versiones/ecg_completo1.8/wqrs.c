...
int8_t ecg_detection_valid(int8_t fp[BUFLNZIP],uint8_t detection[12],int16_t amplitudes[3]){  
	...
	amplitudes[0]=out[1];
	detection[4]=abs(out[0]-out[2]);
	detection[5]=abs(out[0]-out[3]);
						
	if(combine[0]==1){
		detection[10]=abs(out[0]-out[9]);
		amplitudes[2]=out[10];			
		detection[11]=abs(out[0]-out[11]);
	}else {
		detection[10]= 0;detection[11]= 0; 
		if(combine[0]==2){ correct=9;}
	}
	if(combine[1]==1){
		detection[8]=abs(out[0]-out[6]);
		amplitudes[1]=out[7];					
		detection[9]=abs(out[0]-out[8]);
	...
	if(combine[3]==1){
		...
		detection[6]=abs(out[0]-out[4]);
	}else detection[6]=0;   

	if(combine[2]==1){
		
		detection[7]=abs(out[0]-out[5]);
	} else detection[7]=0;  
