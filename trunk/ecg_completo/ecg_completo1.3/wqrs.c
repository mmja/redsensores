...
{  
	...
	dbg(DBG_USR1, "\%d --> MMF: \%d \%d   ",(from)%BUFLN,descomprime(fp,(from+BUFLN)%(BUFLN),0), mmt((from+BUFLN)%(BUFLN),fp));
...
{  
	...
	//if(correct==1)	TOSH_SET_MISC1_PIN();	else TOSH_CLR_MISC1_PIN();
	//return 1;
	...
	if (correct==1){
		...
		dbg(DBG_USR1, "Rpeak not detected\n"); 
		return 0;	
	}
...
{  
	...
	// Step 4: Rwave detection		
        ...
        dbg(DBG_USR1, "Rwave not detected\n");  
...
{  
	int8_t correct=0; // comprobamos si cada paso es correcto (correct =0) o si ha fallado (correct =1)	
			...
			//dbg(DBG_USR1, "\%d --> MMF:  \%d \%d \%d %d %d %d %d \%d \%d %d \%d \%d  %d\n",(from)%BUFLN,descomprime(fp,(from+BUFLN)%(BUFLN),0), mmt((from+BUFLN)%(BUFLN),fp),out[0],out[2],out[3],out[4],out[5],out[6],out[7],out[8], out[9], out[10], out[11]);	
				...
				dbg(DBG_USR1, "\%d %d %d %d %d \%d \%d %d \%d \%d  %d\n",out[0],out[2],out[3],out[4],out[5],out[6],out[7],out[8], out[9], out[10], out[11]);	
			      		
				    	return correct;
				}
				else dbg(DBG_USR1, "\%d %d %d %d %d \%d \%d %d \%d \%d  %d\n",out[0],out[2],out[3],out[4],out[5],out[6],out[7],out[8], out[9], out[10], out[11]);	
 
				//dbg(DBG_USR1, " hola \%d --> MMF:  \%d \%d \%d %d %d %d %d \%d \%d %d \%d \%d  %d\n",(from)%BUFLN,descomprime(fp,(from+BUFLN)%(BUFLN),0), mmt((from+BUFLN)%(BUFLN),fp),out[0],out[2],out[3],out[4],out[5],out[6],out[7],out[8], out[9], out[10], out[11]);	
