...
int16_t mmt(int16_t current,int16_t f[BUFLN]){
    ...
    return ((max+min-2*f[(current)&(BUFLN-1)])*10 / s);
...
void thresholding(int16_t lista[distance],int16_t min, int16_t max){
	...
	for(i=1;i<(n/groups)+1 && !(aux>=cantidad[i] && cantidad[i]<cantidad[i+1]  &&processedVal>225);i++){
		processedVal+=cantidad[i];
		aux=cantidad[i]; 		
	}
	thf=valores[i];
...
int8_t ecg_detection(int16_t datum, int16_t buffer[BUFLN],int16_t out[12])
{ 
	...
	/*if(count==0){
		...
		notnoise=notnoise&(BUFLN-1);
		for(i=0;i<distance;i++){
		
			lista[i]=abs(mmt((i+from)&(BUFLN-1),fp));
		}
	...
	if (correct==1){	
		...
		correct =-1;
      	correct=rwave(fp);
if(luz==100){TOSH_TOGGLE_GREEN_LED_PIN(); //TOSH_SET_MISC1_PIN();
TOSH_TOGGLE_GREEN_LED_PIN();
	...
	// Step 7: onset and offset, Pwave and Twave detection
			...
			out[0]=Rpeak&(BUFLN-1);out[1]=fp[Rpeak&(BUFLN-1)];out[2]=Rwave[0]&(BUFLN-1);out[3]=Rwave[1]&(BUFLN-1);

			if(combine[3]==1)out[4]=Qwave&(BUFLN-1); else out[4]=0;
			if(combine[2]==1)out[5]=Swave&(BUFLN-1); else out[5]=0;
			if(combine[1]==1){  out[6]= Pwave[0]&(BUFLN-1);out[7]= fp[Pwave[2]&(BUFLN-1)];out[8]= Pwave[1]&(BUFLN-1);}else {out[6]= 0;out[7]=0;out[8]=0;}
			if(combine[0]==1){out[9]=Twave[0]&(BUFLN-1);out[10]= fp[Twave[2]&(BUFLN-1)];out[11]= Twave[1]&(BUFLN-1);}else {out[9]=0;out[10]= 0;out[11]= 0;}
			//dbg(DBG_USR1, "\%d --> MMF: \%d \%d \%d \%d %d %d %d %d \%d \%d %d \%d \%d  %d\n",from&(BUFLN-1),buffer[(from)&(BUFLN-1)],fp[(from)&(BUFLN-1)], mmt((from)&(BUFLN-1),fp),out[0],out[2],out[3],out[4],out[5],out[6],out[7],out[8], out[9], out[10], out[11]);	
