...
int16_t count=0, init=0, from=0;//main circle buffer indexs 
...
int8_t aux1[BUFLNZIP],aux2[BUFLNZIP]; //auxiliar buffers to do the noise filtering
int8_t fp[BUFLNZIP]; //filtered signal
...
void mmf(int8_t f[BUFLNZIP],int8_t sol[BUFLNZIP]){
	...
	//B, Bo (oppening) y Bc (closing)
	static int8_t Bc[l1];//we reuse the Bc buffer to all structural elements
...
//********************************************************************************************
//the firts two local maxima from Rpeak to the right and to the left
...
int8_t pwave(int8_t f[BUFLNZIP], int16_t outecg[12]){	
	...
	if(combine[3]==1){ // if Qwave does exist 
		offsetP=(detec[3]-1);
		pPeak=detec[3]-2;
		onsetP=(detec[3]-3);
	}else{				// if Qwave doest exist 
		offsetP=(detec[1]-1);
		pPeak=detec[1]-2;
		onsetP=(detec[1]-3);
    }
...
int8_t twave(int8_t f[BUFLNZIP], int16_t outecg[12]){
	...
	if(combine[2]==1){ // if Swave does exist 
		onsetT=(detec[4]+1);
		offsetT=(detec[4]+3);
		tPeak=detec[4]+2;
	}
	else{				// if Swave doest exist 
		onsetT=(detec[2]+1);
		offsetT=(detec[2]+3);
		tPeak=detec[2]+2;
    }
...
void thresholding(int16_t n){
	...
	for(i=0;i<=n/GROUPS;i++) cantidad[i]=0;
	
	//histogram
	for(i=0;i<POINTS;i++){
		
		cantidad[abs(values[((i+countdet)+POINTS)%(POINTS)])/GROUPS]++;
	}
	
	//The two between-peak valleys in the histogram gave rise to the values of ThR and Thf.
	aux=cantidad[n/GROUPS];
	processedVal=cantidad[n/GROUPS];
	
	for(i=(n/GROUPS)-1;i>0 && !(aux>=cantidad[i] && cantidad[i]<cantidad[i-1]);i--){
	...
	for(i=1;i<(n/GROUPS)+1 && !(aux>=cantidad[i] && cantidad[i]<cantidad[i+1]);i++){
		processedVal+=cantidad[i];
		aux=cantidad[i]; 		
	}
	thf=i*GROUPS;
...
void giveTime(int16_t instant,int8_t rtime[12]){
	...
	else pos-=(count+(BUFLN*filled_buffer_fp) + BUFLNVIRT- instant)%BUFLNVIRT; 
	while(pos<0){
		pos+=200;
		rs--;
	}
...
int8_t mainPoints( int8_t f[BUFLNZIP]){
	...
	if(((val1==val2) && (val1>datum || val1<datum))){
		valorAnterior=datum;
		}
	
	//if val1 is maximum or minimum and it is bigger than thf, we save it 
	if(((val1>val2 && val1>datum)||(val1<val2 && val1<datum)) ){
	...
	//if val1 is the last value of several identical values, and it is bigger than thf, we save it
		if(((val1==datum && valorAnterior<val1 && val1>val2)||(val1==datum && valorAnterior>val1 && val1<val2)) ){
			values[countdet]= val1;
			valuesfp[countdet]=descomprime(f,(j+1 +BUFLN)%(BUFLN),0);   
			//saving the position respect the virtual buffer with 200 longitude
			if( (j+1 +BUFLN)%(BUFLN)!=0 )maxMin[countdet]= (j+1+BUFLN)%(BUFLN)+BUFLN*filled_buffer_fp;
...
int8_t ecg_detection_datain(int16_t datum, int8_t buffer[BUFLNZIP])
{  static uint8_t detection[12];
	...
	mmf(buffer,fp); //morfological filtering
...
//****************************************************************************************************************************************
int8_t ecg_detection_rpeak(int8_t buffer[BUFLNZIP],uint8_t detection[12]){ 
	int8_t correct=0; 
	int16_t max,i; //thresholding
	if(notnoise>countdet){
		...
		//Thresholding calculation
			...
			max=0;
			for(i=0;i<POINTS;i++){	
				if(max<abs(values[((i+countdet)+POINTS)%(POINTS)]))max=abs(values[((i+countdet)+POINTS)%(POINTS)]);
			}
			thresholding(max);
...
//****************************************************************************************************************************************
int8_t ecg_detection_rwave(int8_t buffer[BUFLNZIP]){  	
	...
	// Step 4: Rwave detection		
      	...
      	if (correct==1) {    
        	return 1;}	//Rwave detected
        
	return 0;
}
//****************************************************************************************************************************************
int8_t ecg_detection_qwave(int8_t buffer[BUFLNZIP]){     
...
//****************************************************************************************************************************************
int8_t ecg_detection_swave(int8_t buffer[BUFLNZIP]){       
	// Step 6: Swave detection
	combine[2]= swave(fp,out);	
	return 1;
}
//****************************************************************************************************************************************
int8_t ecg_detection_pwave(int8_t buffer[BUFLNZIP]){  
...
//****************************************************************************************************************************************
int8_t ecg_detection_twave(int8_t buffer[BUFLNZIP]){  	
		// Step 7: onset and offset, Pwave and Twave detection
...
//****************************************************************************************************************************************
int8_t ecg_detection_valid(int8_t buffer[BUFLNZIP],uint8_t detection[12],int16_t amplitudes[3]){  
	int8_t correct=0; 
