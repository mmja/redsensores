// VERSION 2.2
// Envio - Recepcion: Se queda un tiempo sin enviar nada (dormido)   
...
implementation {
	...
	switch(cycle){
		...
		case 8:data[j++] = get_sample_from_core(); 
			...
			if(result!=resultAnterior && sleepSend < (SLEEP>>1)){
			...
			if(sleepSend >= (SLEEP>>1))resultAnterior=-1;
			else resultAnterior=result;
			result=0;
			cycle=0;
			break;
