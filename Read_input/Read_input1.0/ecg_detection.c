...
int16_t mmt(int16_t current,int8_t f[BUFLNZIP]){
    ...
    if ((current < (from)) && (current > (from)+DISTANCE) ) {
		...
		return descomprime(f,(current+BUFLN)%(BUFLN),0);
}	
...
{  
	...
	if (correct==1){
		...
		// Step 7: onset and offset, Pwave and Twave detection
				...
				if(combine[0]==1){
					if(filled_buffer==0)correct=0;//validation(out,((out[0]-last_Rpeak+BUFLN)%BUFLN));
					else correct=0;//validation(out,((out[0]-last_Rpeak+BUFLN)%BUFLN)+(filled_buffer-1)*BUFLN);
