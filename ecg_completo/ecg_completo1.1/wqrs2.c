...
int16_t count=0, init=0;//,to=0, from=0,;  //readed values number
int16_t notnoise=0;
...
void mmf(int16_t f[BUFLN],int16_t sol[BUFLN]){
	...
	//formula 2
	dilation(sol,l1,B1,aux);
	erosion(aux,l1,B2,first);
	//(void)free(aux);
	
	erosion(sol,l1,B1,aux);
...
int16_t mmt(int16_t current,int16_t f[BUFLN]){
    ...
    if ((current < (init+margin)) && (current > (init+margin)+distance) ) {
    ...
    //while(tt<=current){
		...
		for (t = current-s; t <= current+s; t++){ //find the maximum and minimum local values 
			...
			//if((aux=getsample(t,f))!= WFDB_INVALID_SAMPLE){
				if (f[(t)&(BUFLN-1)] > max) max = f[(t)&(BUFLN-1)]; 
				if (f[(t)&(BUFLN-1)] < min) min = f[(t)&(BUFLN-1)];
			//}
			
		}
		

    return ((max+min-2*f[(t)&(BUFLN-1)])*10 / s);
...
int8_t rwave(int16_t f[BUFLN]){
    ...
    for(r=(Rpeak+1);r<((init+margin)+distance) && !(right_local_max<= mmt(r,f) && mmt(r,f)>mmt(r+1,f) && abs(mmt(r,f) )>thf);r++){
	     if(right_local_max< mmt(r,f)&& mmt(r,f)==mmt(r+1,f)){right=r;}		    
	    right_local_max=mmt(r,f);
	}
    //Looking for the first local maximun to left	     
     for(l=(Rpeak-1);l>=(init+margin) && !(left_local_max<=mmt(l,f) && mmt(l,f)>mmt(l-1,f) && abs(mmt(l,f) )>thf);l--){  
	     if(left_local_max< mmt(l,f)&& mmt(l,f)==mmt(l-1,f)){left=l;}	
	    left_local_max=mmt(l,f);	    
	}

    if(r< ((init+margin)+distance) && l>=(init+margin) )
    {	
...
int8_t qwave(int16_t f[BUFLN]){
	...
	// Looking for the first local minumun to left
	 for(l=(Rwave[0]-1);l>=(init+margin) && !(left_local_min>mmt(l,f) && mmt(l,f)<=mmt(l-1,f) && abs(mmt(l,f))>thf)&&(t1!=0);l--,t1--){  
		left_local_min=mmt(l,f);	    
		}

	Qwave=l;	

    	if(l>=(init+margin) && (t1!=0)) {return 1;}else{return 0;}
}
//********************************************************************************************
...
int8_t swave(int16_t f[BUFLN]){
	...
	// Looking for the first local minumun to right
	 for(r=(Rwave[1]+1);r<((init+margin)+distance) && !(right_local_min>mmt(r,f) && mmt(r,f)<=mmt(r+1,f) && abs(mmt(r,f))>thf)&&(t1!=0);r++,t1--){    
		 right_local_min=mmt(r,f);	    
		}
   		Swave=r;	
    	if(r<((init+margin)+distance) && (t1!=0))  {return 1;}else return 0;
}
...
int8_t pwave(int16_t f[BUFLN]){
	...
	while(!encontrado){
		...
		for(left1=(l-1);left1>=(init+margin) && !(onsetP<=mmt(left1,f) && mmt(left1,f)>mmt(left1-1,f)&& mmt(left1,f)>0 /*&& abs(mmt(left1,f))>thf*/ ) &&(t1!=0);left1--,t1--){ 
			onsetP=mmt(left1,f);	    
		}
		//busca minimo intermedio que supere thf
		for(l=left1;l>=(init+margin) && !(mmt(l+1,f)>=mmt(l,f) && mmt(l,f)<mmt(l-1,f) ) &&(t1!=0) ;l--,t1--);
		//l=left1;
		if(l<(init+margin) || (t1==0))return 0;
	...
	for(left2=l;left2>=(init+margin) && !(offsetP<=mmt(left2,f) &&mmt(left2,f)>mmt(left2-1,f) && mmt(left2,f)>0/*&& abs(mmt(left2,f))>thf*/) &&(t1!=0);left2--,t1--){ 
		offsetP=mmt(left2,f);	    
	}

    if(left1>=(init+margin) && left2>=(init+margin)  &&(t1!=0))
    {		
		  Pwave[0]=left2;//onset
...
int8_t twave(int16_t f[BUFLN]){
	...
	while(!encontrado){
		...
		for(right1=r+1;right1<((init+margin)+distance) && !(onsetT<=mmt(right1,f) &&mmt(right1,f)>mmt(right1+1,f) && mmt(right1,f)>0/*&& abs(mmt(right1,f))>thf*/) &&(t1!=0);right1++,t1--){
			onsetT=mmt(right1,f);	    
		}
		//busca minimo intermedio 
		for(r=right1;r<((init+margin)+distance) && !(mmt(r-1,f)>=mmt(r,f) && mmt(r,f)<mmt(r+1,f))&&(t1!=0);r++ ,t1--);
		
		//si no ha encontrado ningun minimo y se ha salido
		if(r>=((init+margin)+distance) || (t1==0))return 0;
	...
	offsetT=mmt(r,f);
	 for(right2=(r+1);right2<((init+margin)+distance) && !(offsetT<=mmt(right2,f) && mmt(right2,f)>mmt(right2+1,f) && mmt(right2,f)>0/* && abs(mmt(right2,f))>thf*/)&&(t1!=0);right2++,t1--){    
	   offsetT=mmt(right2,f);	    
		}
		
    if(right1< ((init+margin)+distance) && right2< ((init+margin)+distance)  &&(t1!=0))
    {		
...
void thresholding(int16_t lista[distance],int16_t min, int16_t max){
	...
	//=(int16_t *)malloc((to-(init+margin))*sizeof(int16_t));
	...
	for(i=1;i<(n/groups)+1 && !(aux>=cantidad[i] && cantidad[i]<cantidad[i+1]  &&processedVal>250);i++){
		processedVal+=cantidad[i];
		aux=cantidad[i]; 		
	}
	thf=valores[i];
...
int32_t wqrs(int16_t datum, int16_t buffer[BUFLN],int16_t out[12])
{ 
	...
	//from=init+55;//+NOPS*(1.5*LQRS*FS-1)/2;
	//to=from+distance;
	...
	if(count==0){
		...
		for(i=0;i<distance;i++){
			...
			lista[i]=abs(mmt((i+init+margin)&(BUFLN-1),fp));
		}
	...
	if(notnoise>=init+margin){
		dbg(DBG_USR1, "\%d --> MMF: \%d \%d \%d\n",init+margin,buffer[(init+margin)&(BUFLN-1)],fp[(init+margin)&(BUFLN-1)], mmt((init+margin)&(BUFLN-1),fp));
		return 0;	
	}
	...
	// Step 7: onset and offset, Pwave and Twave detection
			...
			dbg(DBG_USR1, "\%d --> MMF: \%d \%d \%d \%d %d %d %d %d \%d \%d %d \%d \%d  %d\n",init+margin,buffer[(init+margin)&(BUFLN-1)],fp[(init+margin)&(BUFLN-1)], mmt((init+margin)&(BUFLN-1),fp),out[0],out[2],out[3],out[4],out[5],out[6],out[7],out[8], out[9], out[10], out[11]);	
